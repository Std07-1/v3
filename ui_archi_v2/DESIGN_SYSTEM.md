...
**Доступні:** `chat, feed, thinking, relationship, mind, workspace, logs, bell, belloff, mic, smile, send, attach`
**Де застосовано** (`App.svelte`): sidebar nav (desktop) · mobile bottom-nav (єдина мобільна навігація — 7 іконок; ☰ section-switcher видалено) · notif-toggle. Чат: mic/smile (composer), send (літачок).
...
- **InputBar** (`InputBar.svelte`) — преміум композер (ADR-0053).
  - **Layout (консистентний, без стрибка):** mic/emoji ЗАВЖДИ зліва (`.lead`), send ЗАВЖДИ справа — у ОБОХ станах. Короткий: `[lead][text][send]` один ряд. Довгий (`.pill.multiline`, текст >~1.5 рядки): `flex-wrap` → текст на всю ширину 1-м рядом (`.ta order:-1; flex:1 1 100%`), `[lead … send]` 2-м рядом. Кнопки не «тікають» при наборі.
  - **Анти-halation:** `.pill.multiline .ta` НЕ `flex:1` у колонці (колапсує висоту/ріже текст) — тут row-wrap, height з auto-grow.
  - **Кнопки:** компактні SVG-іконки (mic/smile через `Icon.svelte`), send = `attach`-стиль паперовий-літачок, мініатюрний (28px), accent коли armed.
- **EmojiPicker** (`EmojiPicker.svelte`) — панель сама обирає бік (`toggle` вимірює якір: кнопка в лівій половині viewport → `align-left` відкрити праворуч, інакше `align-right`), `bottom: calc(100%+8px)`, `width: min(280px, 100vw-24px)`. **Outside-dismiss:** `$effect` додає `pointerdown` (поза `.emoji-anchor` → закрити) + Esc; containment — тап усередині панелі НЕ закриває.
...
### Можливості чату — Крок 1 (DONE 2026-06-07)

- ✅ **«Арчі думає»** (`Chat.svelte`, `awaitingReply && !hasStreamingBubble`) → mood-tinted баббл + **shimmer-sweep** по тексту (`thinkShimmer`, світло пробігає) + хвиля-крапки. Жива анімація, не статика.
- ✅ **Entrance** — `class:fresh` (`ts_ms > mountedAt`) → `msgIn` fade+slide. Тільки НОВІ повідомлення, не історія (нема каскаду на mount).
- ✅ **Scroll-to-bottom** — **центрована** ↓ кнопка (`.scroll-wrap` + `atBottom` через `onScroll`), показується коли не внизу.
- ✅ **Копіювати** — `.btn-copy` на архі-баблах (`opacity 0.4` завжди → **тапабельно на мобільному**), clipboard + ✓ feedback.

**Крок 1+ далі:** scenario/VP картка · tappable рівні (⚠️ **тільки пасивні дії: копіювати / показати на чарті — НЕ викликати Архі**, щоб не смикати зайвим викликом) · кнопка→чарт · pinned.

**Велике окремо (ADR): чарт-міст** — шар Архі на чарті + двостороннє тап↔підсвітка + аналіз→shareable картинка для каналу.
...
- ✅ Можливості чату Крок 1 (думає-shimmer · entrance · scroll-to-bottom · copy) — DONE 2026-06-07
- ⏳ Можливості чату Крок 1+: scenario-картка · tappable рівні (пасивні) · кнопка→чарт · pinned
