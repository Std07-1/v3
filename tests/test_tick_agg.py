...
class TestTickAggregator(unittest.TestCase):
    ...
    def test_tick_aggregator_rejects_htf_tf(self) -> None:
        """H4/D1 в allowlist — відмова: бакет тут від епохи, сезонна сітка лише в HTF-акумуляторі (ADR-0095 S4b)."""
        for allowlist in ([14400], [60, 86400], [60, 180, 14400, 86400]):
            with self.assertRaisesRegex(ValueError, "TICK_AGG_HTF_TF_FORBIDDEN"):
                TickAggregator(tf_allowlist=allowlist)
        agg = TickAggregator(tf_allowlist=[60, 180, 300, 900, 1800, 3600])
        _, h1 = agg.update("XAU/USD", 3600, 1_700_000_000_000, 100.0)
        self.assertIsNotNone(h1)
        if h1 is None:
            return
        self.assertEqual(h1.open_time_ms, 1_700_000_000_000 - 1_700_000_000_000 % 3_600_000)
