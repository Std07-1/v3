...
`--out` (рядки staging) і `--result` (JSON); у staging-доби переносить батько після повторної валідації.
...
def main(argv: Optional[List[str]] = None, provider_factory: Optional[Callable[[Dict[str, Any]], Any]] = None) -> int:
    ...
    try:
        raw_rows = _fetch_raw(args.symbol, day, provider_factory)
...
def _fetch_raw(symbol: str, day: dt.date, provider_factory: Optional[Callable[[Dict[str, Any]], Any]]) -> list:
