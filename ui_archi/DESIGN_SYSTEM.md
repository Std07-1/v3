...
- **InputBar** (`InputBar.svelte`) — преміум композер (ADR-0053). **Multiline-режим:** текст >~1.5 рядки → `.pill.multiline` (текст на ВСЮ ширину, кнопки mic/emoji/send рядком ПІД ним) — зручно писати довгі повідомлення на мобільному. Короткий текст → компактний інлайн.
