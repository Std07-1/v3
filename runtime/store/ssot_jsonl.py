...
def read_tail_bars(data_root: str, symbol: str, tf_s: int, n: int) -> List[CandleBar]:
    """Читає останні n барів з диску (tail) для (symbol, tf_s)."""
    if n <= 0:
        return []
    sym_dir = symbol.replace("/", "_")
    tf_dir = f"tf_{tf_s}"
    ...
    def iter_lines_reverse(path: str):
        try:
    ...
    for part in reversed(parts):
        path = os.path.join(dir_path, part)
