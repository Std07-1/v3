<!--
  src/layout/AboutModal.svelte — ADR-0066 PATCH 04
  Modal with About + Credits tabs.
  Header: Brand lockup + version line + build-date (from Vite env).
  Esc + backdrop close. Focus trap on open. Theme-aware via tokens.
-->
...
<script lang="ts">
    ...
    const VERSION =
        (typeof __APP_VERSION__ !== "undefined" ? __APP_VERSION__ : "v3-dev") as string;
    const BUILD_DATE =
        (typeof __BUILD_DATE__ !== "undefined" ? __BUILD_DATE__ : "dev") as string;
    const COMMIT_HASH =
        (typeof __COMMIT_HASH__ !== "undefined" ? __COMMIT_HASH__ : "—") as string;

    let activeTab: "about" | "credits" = $state("about");
                ...
                {#if activeTab === "about"}
                    <div class="tab-pane">
                        ...
                        <p class="about-blurb">
                            ...
                            platform for institutional order-flow trading.
                            Smart Money Concepts overlay (zones, structure,
                            liquidity, P/D) with an autonomous AI agent
                            (Archi) producing real-time narrative and
                            scenario synthesis.
                        </p>
                        <p class="disclaimer">
                ...
                {:else}
                    <div class="tab-pane">
                        <p class="credits-intro">
                            ...
                            attributions are reproduced below per each
                            project's terms.
                        </p>
                        <ul class="oss-list">
                            {#each OSS_NOTICES as notice}
                                <li class="oss-item">
                                    <div class="oss-head">
                                        ...
                                        >{notice.name}</a>
                                        <span class="oss-license">{notice.license}</span>
                                    </div>
                                    <div class="oss-copy">{notice.copyright}</div>
                                    <div class="oss-purpose">{notice.purpose}</div>
                                    {#if notice.noticeText}
                                        <pre class="oss-notice">{notice.noticeText}</pre>
                                    {/if}
                                </li>
                            {/each}
                        </ul>
...
<style>
    ...
    .close-btn:hover { color: var(--text-1); background: color-mix(in srgb, var(--accent) 8%, transparent); }
    ...
    .version-line strong { color: var(--text-1); }
    .version-line code {
        font-family: var(--font-mono);
        color: var(--accent);
    }
    ...
    .oss-name:hover { text-decoration: underline; }
    .oss-name:focus-visible { outline: 2px solid var(--accent); outline-offset: 1px; }
    .oss-license {
        font-family: var(--font-mono);
        font-size: var(--t6-size);
        font-weight: 600;
