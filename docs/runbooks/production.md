...
1. Python ≥3.11 venv активований (`.venv`). Python 3.7 (`.venv37`) — тільки для FXCM broker sidecar (ADR-0016)
...
2. **M1 Poller** (M1/M3 pipeline, FXCM History via broker sidecar):

   ```bash
   python -m app.main --mode m1_poller --stdio files --log-dir logs
   ```

3. **Tick Publisher** (тік-стрім FXCM):

   ```bash
   python -m app.main --mode tick_publisher --stdio files --log-dir logs
   ```

4. **Tick Preview Worker** (агрегація тіків → preview):

   ```bash
   python -m app.main --mode tick_preview --stdio files --log-dir logs
   ```

5. **Binance Ingest Worker** (BTCUSDT/ETHUSDT M1 ingest, ADR-0037):

   ```bash
   python -m app.main --mode binance_ingest_worker --stdio files --log-dir logs
   ```

6. **Binance Tick Publisher** (BTCUSDT/ETHUSDT tick stream, ADR-0037):

   ```bash
   python -m app.main --mode binance_tick_publisher --stdio files --log-dir logs
   ```

7. **WS Server** (чекає `prime_ready`):

   ```bash
   python -m app.main --mode ws_server --stdio files --log-dir logs
   ```

> **Важливо**: у `--mode all` supervisor стартує всі 6 процесів і чекає `prime_ready` перед запуском WS Server. У ручному режимі переконайтесь, що m1_poller завершив bootstrap (prime Redis) **до** старту WS Server.
>
> **Примітка**: Connector (`--mode connector`) депрекейтнутий (ADR-0023). H4/D1 тепер derived з M1.
...
|-----------|---------|---------|:---------:|------------|
| **critical** | m1_poller, m1_ingestion_worker, broker_sidecar, binance_ingest_worker, replay | 10s→…→300s | 5 | **supervisor fail** (kills all) |
| **non_critical** | tick_publisher, tick_preview, binance_tick_publisher | 5s→…→120s | 10 | видалено з пулу |
| **essential** | ws_server | 5s→…→120s | 10 | видалено з пулу |
...
Перевірка: disk TF-файли оновлені для всіх 4 символів (XAU/USD, XAG/USD, BTCUSDT, ETHUSDT).  
...
| --- | --- | --- |
| `status.prime_ready` | `true` | `false` → m1_poller ще bootstrapping |
...
- UI не стартував (m1_poller ще bootstrapping → `prime_ready=false`)
...
- Якщо critical (m1_poller/binance_ingest): виправити root cause (corrupt JSONL, FXCM/Binance down, Redis down), потім перезапустити.
...
- m1_poller ще не завершив bootstrap (prime_ready=false)
...
Автоматичний: m1_poller detect gap → `_live_recover_check()` → dosyl missing bars. Деталі: [runbooks/live_recover.md](live_recover.md).

---

## DST перехід (літній/зимовий час)

FX/CFD ринки прив'язані до нью-йоркського часу. При US DST потрібно оновити якорі H4/D1 і calendar values в `config.json`, а потім перебудувати H4/D1 бари.
...
| Запускати UI без m1_poller bootstrap | Порожній графік | `--mode all` або дочекатись prime_ready |
| Додавати `prefer_redis`/`force_disk` в UI код | Порушення I1 (UDS = вузька талія, Тема D, D6) | Сервер обирає джерело |
| Міняти `config.json` під час роботи | Частково підтримується (mtime cache), але m1_poller потребує рестарту | Зупинити → змінити → запустити |
| Ігнорувати DST перехід | H4/D1 бари з garbage alignment | [DST runbook](dst_transition.md) |
