...
> **СКАСОВАНО ADR-0095 S5c (23.09.2026): кроки якорів H4/D1 не виконувати.** Якір — функція дати (`htf_anchor`,
> 17:00 America/New_York), сітка H4/D1 сама переходить через вихідні DST. Ключі `day_anchor_offset_s*` і
> `binance.*_anchor_offset_s` прибрано з config: їх поява дає `ValueError CONFIG_LEGACY_ANCHOR_KEY`, і інжест, preview
> та bridge не стартують. Застаріли розділи «config.json — Day Anchors», кроки 3–5 процедури (видалення й rebuild
> H4/D1, очікувані години H4), рядки про `anchor_offset` і `binance.day_anchor_offset_s` у «Що НЕ змінюється» та
> «Архітектурний контекст». До S6b ранбук перемикає **лише плоскі поля календарних груп** (блоки `summer`/`winter` не
> чіпати): EU — 25.10, US — 01.11. Повний перепис — ADR-0095 S8, після вікна міграції, до 25.10.
...
> **СКАСОВАНО ADR-0095 S5c — не виконувати.** Ключі нижче прибрано з config; їх поява дає
> `CONFIG_LEGACY_ANCHOR_KEY`, і старт інжесту, preview та bridge відмовить. Сітка H4/D1 переходить сама.
> До S6b перемикати лише календарні групи нижче. Перепис — ADR-0095 S8.
...
Змінити **лише** плоскі поля календарних груп (див. таблиці вище). Якорі не змінювати: ключів більше нема
(ADR-0095 S5c).

#### 3. Видалити H4/D1 файли від дати переходу DST

> **СКАСОВАНО ADR-0095 S5c — кроки 3–5 не виконувати.** Сітка H4/D1 на сезонному якорі, видаляти й
> перебудовувати нічого. Очікувані години H4 у кроці 5 — стара статична сітка; перевірка сітки — health
> `off_season_grid` і exit gate `d1_anchor_alignment`. Перепис — ADR-0095 S8.
...
## Європейські CFD змінюються за EU DST (окремі дати!)

Виміряно на історії брокера 15.09.2026 (зонд `reopen`/`eu_cfd_dst_probe`): раніше тут було написано, що
європейські CFD мають фіксований розклад — **це хибно**.

| Група | Літо (EU DST) | Зима |
|---|---|---|
| `cfd_eu_eustx50` (EUSTX50) | сесія 06:00–20:00 UTC → break `20:00`–`06:00`, вихідні Пт 20:00 → Пн 06:00 | сесія 07:00–21:00 → break `21:00`–`07:00`, вихідні Пт 21:00 → Пн 07:00 |
| `cfd_eu_ger30` (GER30) | сесія 00:30–20:00 UTC → break `20:00`–`00:30`, вихідні Пт 20:00 → Пн 00:30 | сесія 00:30–**21:00** → break `21:00`–`00:30`, вихідні Пт 21:00 → Пн 00:30 (відкриття лишається 00:30 UTC цілий рік!) |

**Дати EU DST ≠ US DST**: EU перемикається 2026-10-25 і 2027-03-28, US — 2026-11-01 і 2027-03-14. У вікні між
ними якорі H4/D1 (US-прив'язані) вже/ще іншого сезону — це нормально: обидва якорі лишаються всередині break
європейських груп. Порядок кроку: правити календарні групи **до** межі ринку (інакше health бачить дні
попереднього сезону новим календарем).
