...
- **Обрізані доби.** Кожна доба staging несе `trading_minutes_expected` і `coverage` (формат `ft_m1_staging_day_v2`).
  Доба з покриттям нижче `--day-coverage-min` (дефолт 0.80) друкується як `FT_FETCH_DAY_LOW_COVERAGE`, і
  `--only-missing` забирає її ще раз (`FT_FETCH_DAY_LOW_COVERAGE_REFETCH`) — до `--max-day-attempts` спроб (дефолт 2),
  після чого приймається як є (святкові й тонкі доби). Доби staging формату v1 інструмент не читає — перезабрати.
