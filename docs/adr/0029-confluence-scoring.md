...
| ER-7 | Step 7 bar_dicts: `b.l` замість `b.low` → `AttributeError: 'CandleBar' has no attribute 'l'` [VERIFIED engine.py:577]. Wire dict `l` ≠ dataclass `.low`. Exception caught silently (`_log.debug`) → empty overlay on ALL TFs after restart | S0 | `b.l` → `b.low` в engine.py:577. Promoted `_log.debug` → `_log.warning` в smc_runner.py + ws_server.py (I5: degraded-but-loud). Додано X13/Z7 в governance docs | ✅ Applied 2026-03-06 |
| ER-8 | `SmcZone.to_wire()` не включає `anchor_bar_ms` → F1/F2/F4 scorer отримує anchor=0 → sweep/fvg/extremum НІКОЛИ не спрацьовують [VERIFIED types.py:74-87, engine.py:582] | S1 | Engine додає `z_wire["anchor_bar_ms"] = z.anchor_bar_ms` перед scoring (server-side only, UI wire S6 незмінний) | ✅ Applied 2026-03-06 |
| ER-9 | E3 використав `swing_high`/`swing_low` — ці kinds не існують в engine. Реальні: `hh`/`lh` (highs) + `ll`/`hl` (lows) [VERIFIED swings.py:43,56 + structure.py:48-68] → F1/F4/F6 фільтрують нуль свінгів | S1 | `_SWING_HIGHS = {"hh", "lh", "swing_high"}`, `_SWING_LOWS = {"ll", "hl", "swing_low"}` в confluence.py F1/F4/F6 (backward-compat з тестами) | ✅ Applied 2026-03-06 |
| ER-10 | F3 `_check_htf_alignment()` фільтрує `status == "active"` — але реальні HTF зони швидко стають `partially_filled` (FVG) або `tested` (OB). `context_stack.py:98` і `engine.py:116` вважають ці статуси "живими", а F3 — ні → htf_align НІКОЛИ не спрацьовує для реальних зон [VERIFIED confluence.py:71, engine.py:116, context_stack.py:98, order_blocks.py:188, fvg.py:136] | S1 | `_HTF_ALIVE_STATUSES = frozenset(("active", "tested", "partially_filled"))` замість `== "active"` в F3 | ✅ Applied 2026-03-06 |
| ER-11 | **PATCH A-1**: `get_display_snapshot()` step 3 — якщо base TF snapshot має `trend_bias=None`, а HTF snapshot має bias → bias не пропагується. M15 (base=900) з swing_period=3 може мати bias=None → трейдер бачить None замість HTF bias [VERIFIED engine.py:505-506] | S2 | `if snap.trend_bias is None and htf_snap.trend_bias is not None:` → `snap = dataclasses.replace(snap, trend_bias=htf_snap.trend_bias)` після step 3 HTF structure injection | ✅ Applied 2026-03-07 |
| ER-12 | **PATCH A-2**: `_filter_for_display()` — FVG зони зі status=`filled` проходять через display filter → мертві зони (100% filled) показуються трейдеру як активні [VERIFIED engine.py:770-773] | S2 | `z.status != "filled"` guard у FVG zone collection в `_filter_for_display()` | ✅ Applied 2026-03-07 |
| ER-13 | **PATCH A-3**: `get_display_snapshot()` steps 4+5 — step 4 інжектує HTF FVG зони, step 5 (Context Stack) може ін'єктувати ті самі зони повторно → duplікати у display [VERIFIED engine.py:522-523, 537] | S2 | `_step4_zone_ids = frozenset(z.id for z in extra_fvg)` перед step 5 + `htf_zones = [z for z in htf_zones if z.id not in _step4_zone_ids]` в step 5 | ✅ Applied 2026-03-07 |
| ER-14 | **PATCH A-4**: `_filter_for_display()` — FVG зони не мали distance cap → далекі FVG (>10 ATR) відбирали місце у бюджеті в proximity-relevant зон. [VERIFIED engine.py:768-769] | S3 | `fvg_max_dist = disp.proximity_atr_mult * 1.5 * atr` (~9 ATR cap). FVG за межами → відсіяні | ✅ Applied 2026-03-07 |
| ER-15 | **PATCH B**: F2 `_check_fvg_after()` — тільки time adjacency (3-bar window). Реальні FVG що ціново перекриваються з OB (displacement gap → immediate OB formation) не отримували +2, бо anchor далі ніж 3 бари [VERIFIED confluence.py:51-76] | S2 | Додано Variant B: price overlap check (`fvg_lo < ob_hi and fvg_hi > ob_lo`). Будь-який (A time OR B price) → +2. R_TRADER: ob_bear B(4) → A(6) | ✅ Applied 2026-03-07 |
