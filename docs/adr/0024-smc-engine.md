...
### 18.9 PATCH C: TF-Adaptive Config via `tf_overrides` (2026-03-07)

#### Проблема

Глобальний `swing_period=5` оптимальний для H1+ (виявляє major swings), але на M15 (900s) — занадто грубий. Результат: M15 compute snapshot = 0 native зон. `get_display_snapshot()` заповнює M15 display цілком HTF injection (H1+H4 зони) → трейдер бачить на M15 те саме що на H1. Cross-TF drill-down неможливий.

**Root cause** [VERIFIED engine.py:664, config.py:241]: `_compute_snapshot()` використовує `self._config` (один глобальний SmcConfig для всіх TF).

#### Рішення: `for_tf()` + `config.json:smc.tf_overrides`

**S5 SSOT**: всі per-TF параметри — в `config.json`, не hardcoded.
...
"smc": {
  ...
  "tf_overrides": {
    "900": {
      "swing_period": 3,
      "ob": { "min_impulse_atr_mult": 1.0 },
      "fvg": { "min_gap_atr_mult": 0.15 }
    }
  }
}
```

**Механізм** [VERIFIED config.py:259-295]:

1. `SmcConfig.from_dict()` зберігає `_tf_overrides = d.get("tf_overrides", {})`
2. `for_tf(tf_s)` → shallow merge: scalars replaced, sub-dicts rebuilt from `base_dict.update(override_dict)`
3. Якщо override відсутній → повертає `self` (zero allocation)

**Інтеграція** [VERIFIED engine.py:664]:

```python
cfg = self._config.for_tf(tf_s)  # per-TF config, S5 SSOT
```

Всі detector calls (`detect_raw_swings`, `detect_order_blocks`, `detect_fvg` тощо) використовують `cfg`.

#### Підтримувані override-и

| Scope | Поля | Тип |
|-------|------|-----|
| Scalar | `swing_period`, `lookback_bars`, `max_zones_per_tf`, `max_zone_height_atr_mult` | top-level replace |
| Sub-config | `ob`, `fvg`, `structure`, `levels`, `inducement` | dict merge (base + override) |

#### Результат (XAU/USD, 2026-03-07)

| TF | Bias | Native zones | Total | Before PATCH C |
|----|------|-------------|-------|----------------|
| M15 | bullish | 4 (FVG) | 11 | 0 native, 9 HTF-only |
| H1 | bearish | 5 (OB+FVG) | 9 | same |
| H4 | bullish | 4 (FVG) | 5 | same |

#### Інваріанти

- **S5**: Всі thresholds з config.json, zero hardcode
- **S0**: `for_tf()` = pure function, no I/O
- **S2**: Deterministic — same config + same bars = same output
- **I0**: Зміна тільки в core/ (config.py, engine.py) + config.json

#### Rollback

1. Видалити `tf_overrides` з `config.json:smc`
2. Revert `for_tf()` в `config.py`
3. Revert `cfg = self._config.for_tf(tf_s)` → `self._config` в `engine.py:664`
