...
| [0073](ADR-0073-price-scale-overlay.md) | Price Scale Overlay — Custom Canvas Labels for Edge-to-Edge Candles | **Accepted (rev 2)** | 2026-05-11 (rev 2: 2026-05-12; ACCEPTED: 2026-05-12) | Hide LWC `rightPriceScale.visible:false`, render periodic price labels + last-value chip (with dashed connector — V1) + crosshair price label via 4+1 new methods in OverlayRenderer.ts. Rev 2 fixes Sonnet 4.6 deep-review: B1 `series.priceToCoordinate` (not scale), B2 `subscribeCrosshairMove` captures param, H1 `_drawChip` helper signature, H2 `series.priceFormatter()`, H3 hardcoded hex (canvas no CSS vars), L1 drop ADR-0042 cross-ref, L2 attribute `right:64px` to ADR-0065 (not 0070), L3 drop phantom ADR-0070 amendment, A3 dashed connector V1, A4 touch offset, A6 vitest cases. Reclaim ~44px chart width all viewports. Amends ADR-0072 (☰ right:44→8), shifts NarrativeSheet forward-ref to ADR-0074. R3, M3→M4 | `chart_overlay_v1` |
