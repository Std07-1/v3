...
- **R3 thinking mis-attribution** (post-review fix): наївний per-wake nearest-джойн тягнув ОДИН
  thinking-запис на кілька сусідніх пробуджень (сусідній heartbeat без власного thinking
  показував чуже reasoning). Mitigation — `_assign_thinking`: 1:1, кожен запис архіву віддається
  щонайбільше одній (взаємно-найближчій) картці. Залишковий ризик: архів не несе wake_id
  (точного ключа не існує на боці writer-а), тож ts-skew теоретично може приписати запис
  сусідові — задокументований best-effort; справжнє закриття = wake_id у
  `thinking_archive.append` (trader-side, окремий ADR-кандидат).
- **R4 словник call_type** (post-review fix): архів штампує лише три мітки
  [VERIFIED trader core.py:1694/2679/3309] — `reactive`, `proactive` (парасолька для ВСІХ
  проактивних пробуджень), `daily_review`; wake_log пише фактичний call_type
  (`platform_wake`, `observation`, …). Без family-нормалізації (`_archive_call_type`)
  thinking був би систематично `null` саме для alert-карток.
