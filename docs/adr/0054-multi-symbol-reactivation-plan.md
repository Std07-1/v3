...
- **Статус**: **Accepted (rev 3)** — rev 3 (2026-09-10): Фази 0-2 закриті, §3.6 закрито, W1/W2 виконані, додано §3.8 (блокери масштабування, знайдені постфактум).
...
1. **D1-вимір health-check структурно сліпий (S1).** `expected_bucket_opens` вважає бакет очікуваним лише
   якщо торгується його **перша хвилина** (`core/health/measures.py:119`), а D1-якір 75600 s = 21:00 UTC
   лежить усередині денної перерви в **чотирьох із п'яти** груп (`cfd_us_22_23` 21:00-22:00,
   `cfd_eu_21_07` 21:00-07:00, `cfd_hk_main` 19:00-01:15, `fx_24x5_utc_summer` 20:55-21:30). Прогін на
   реальних даних: XAU D1, штучно обрізаний на 30 і на 400 днів, дає `age_buckets=0, holes=0/0` — так само,
   як цілий. Тобто «D1 ok» у звітах W1/W2 означало «ми не перевіряли», і `--compare` на D1 порівнює два
   вакууми. Це та сама мовчазна поломка, заради якої інструмент писався (NAS100 06.09: M1/H4 живі,
   D1 обірваний на два місяці). **Виправити до W3.**
