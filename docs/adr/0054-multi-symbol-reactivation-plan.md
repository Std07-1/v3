...
- **Статус**: **Accepted (rev 5)** — rev 5 (2026-09-17): W5 EUSTX50 активовано (шостий символ), §3.8 п.2 (`has_range`) закрито патчем на проді, календарні групи EU розділені за виміряними годинами; W4 GER30 готовий і чекає вікна. rev 4 (2026-09-10): §3.8 п.1 (сліпий D1-вимір) закрито; решта блокерів відкриті. rev 3: Фази 0-2 закриті, §3.6 закрито, W1/W2 виконані, додано §3.8.
...
| W4 | **GER30** | `cfd_eu_ger30` | 🟢 **ГОТОВИЙ, чекає вікна**: M1 засіяно у FIRST_TICK 19.03–11.09 (144 696 барів, шви відсортовано), група виміряна — літо 00:30–20:00 UTC, зима 00:30–21:00 (відкриття о 00:30 UTC цілий рік; конфігурація `cfd_eu_21_07` 07–21 була хибною). Блокер §3.8 п.2 закрито патчем `_slot_has_trading` (`59d36e3`, на проді з 17.09 21:05): без нього H4 22:00 щодня втрачав 00:30–00:59 (41 із 239 барів хибні на виміряному тижні). Лишилось: досіяти хвіст (14.09→), перебудувати похідні, health, активація в закритому вікні |
| W5 | **EUSTX50** | `cfd_eu_eustx50` | ✅ **ВИКОНАНО 17.09.2026** (`4ceb2d0`, шостий живий символ). Засів M1 у FIRST_TICK 30.03–17.09 (105 465 барів; хвіст досіяно виправленим інструментом), похідні перебудовано порціями (дублікатів 0 у 8 TF, D1=110), шви відсортовано (інверсій 0). Календарна група виміряна на історії брокера: літо 06:00–20:00 UTC (0 хвилин поза календарем на 117 добах), зима 07:00–21:00. Health 7/8 TF ok, root-розбіжностей 0, age 0; M1 RED — 16 хвилин без угод із 1680. Бутстрап: `UDS_PRIME_SUMMARY` × 8 TF (raw == final), `HTF_SEED`, `SMC_WARMUP_M1_OK bars=2880`, `M1_POLLER_REDIS_PRIME symbols=6`; гейт `--compare` без регресій, крім відомого тонкого хвоста US30 (14 хвилин без угод перед перервою, накопичені ДО рестарту) |
...
2. **`has_range` вирішував торговість слота за першою хвилиною (S1). ✅ ЗАКРИТО 17.09.2026** (`59d36e3`, на проді 21:05): єдиний предикат `_slot_has_trading` (слот торговий, якщо в ньому ≥1 торгова хвилина) у `has_range`/`range_bars`/`missing_count`/boundary-tolerant/`expected_count`. Доведено: GER30 H4 з розбіжністю проти M1 41→3 з 239; XAU rebuild старим і новим кодом байт-у-байт на всіх 7 TF; адверсаріальне ревʼю 4 лінзами — S0–S2 нуль. Опис проблеми нижче лишається як історія.
