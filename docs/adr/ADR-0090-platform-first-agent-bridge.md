...
| **S2** WakeEngine у bridge | `runtime/agent_bridge/wake_engine.py` (move 699 LOC) з власним 2-с циклом; SMC-вхід = localhost-only `GET /api/internal/smc_snapshot?symbol=&since_ts_ms=` на ws_server (`allow 127.0.0.1`, не проксюється nginx). **Контракт (RECON 06.09 проти коду, а не за планом)**: `WakeEngine._tick_symbol` бере з `SmcRunner` дев'ять входів — `get_last_price`, `_engine.get_atr` по (14400,3600,86400) *(приватний доступ — на S2 стає публічним полем контракту)*, `get_snapshot` × 3 TF, `get_bias_map`, `get_zone_grades(14400)`, `get_raw_snapshot`+`get_atr` по target-TF з `tf_pairs`, `get_recent_structure_events(since_ts_ms)`, `get_recent_bar_closes(since_ts_ms)`, плюс `_get_session_info` (календар). З самих снапшотів pure-генератори читають вузько: `auto_wake` — лише `snap.zones` (`id/status/high/low`), `structure_forecast` — `snap.swings` + `snap.trend_bias`; серіалізація вже є (`SmcZone.to_wire`, `SmcSwing.to_wire`), `from_wire` треба дописати у bridge (dependency rule дозволяє `runtime/agent_bridge → core`). `since_ts_ms` = стан WakeEngine (`_last_wake_ts`), тому endpoint параметризований, а не «знімок». Оцінка переглянута: **new ≈300** (endpoint ~120 + клієнт/десеріалізація ~100 + цикл ~40 + тести), не ≈100 | move 699; new ≈300 | wake_events у Redis з'являються при вимкненому `ws_server.wake_engine` і ввімкненому bridge; latency wake ≤5 с; round-trip тест: `zones/swings` після `to_wire→from_wire` дають ті самі умови, що in-process |
...
| **S5** Репо-гігієна | **Інвентар RECON 06.09**: у `tools/` **~75** файлів з hardcoded `/opt/smc-trader-v3` — `_archive_patches/` (46, історичний архів одноразових патчів бота), `bot_patches/` (12), плюс `check_archi*`, `check_wake.py`, `check_directives*.py`, `check_config.py`, `check_stats_date.py`, `clean_channel_beliefs.py`, `fix_market_memory.py`, `reset_budget.py`, `reset_breaker.py`, `diag/{archi_health,archi_state_dump,check_conv,kb_audit,_check_timers}.py` (план називав ~10). Жоден не імпортується платформою (grep: єдиний внутрішній імпорт `tools.diag.classify_m5_gaps` — платформенний, лишається). Це **чужий архів у чужому репо** → рішення власника: видалити з платформи (git history тримає) чи перенести копією в `trader-v3/tools/`. Три SPA (`ui_archi`, `ui_archi_v2`, `ui_ochi`) **не чіпати** поки в `ui_archi_v2` живе незакомічений WIP. `gate_ui_no_direct_redis`: зняти ws_server з `ALLOW_FILES` **не можна** до окремого патчу — лишаються два платформні `import redis` (viewer_signal у delta_loop, D1 tick relay) + клієнт для `/api/v3`. Тести з формами бота → `tests/agent_bridge/` | move; new ≈40 | гейт без винятку зелений **лише після винесення платформних redis-імпортів у фабрику**; `pytest tests/agent_bridge` зелений |
...
- 2026-09-06: RECON проти коду перед S2/S5 → обидва плани виправлено (D13.7 «план — гіпотеза»).
  S2: контракт internal-endpoint виявився ширшим за «ціну» (9 входів `SmcRunner`, снапшоти по 3+2 TF),
  оцінка ≈100 → ≈300 new LOC; вузьке місце не в обсязі, а в новому wire-контракті `zones/swings`
  (серіалізація вже є, `from_wire` — ні). Поки Арчі OFF, WakeEngine не має споживача, тому S2 не блокує нічого.
