...
**Межі / попереду:**
- **trend/rect потребують обидва кінці в завантаженому діапазоні** [VERIFIED TrendTool.ts:25] — на TF, де кінець «не дотягнутий» (давня дата на M1 до scrollback), фігура не рендериться, поки не доскролиш. hline (лише ціна) завжди видно. Прийнято (рендер-факт).
