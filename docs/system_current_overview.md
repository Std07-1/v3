...
```text
app.main (supervisor)
  ├── broker_sidecar        (ADR-0016: FXCM M1 fetcher + tick relay V2, .venv37/, Redis IPC)
...
> broker_sidecar (M1 fetch + tick relay V2), і `.venv/` (Python ≥3.11) для всього іншого.
> `tick_publisher_fxcm` — **зупинений назавжди** (FXCM SDK не підтримує дві сесії з одного акаунту).
> Тіки FXCM тепер йдуть через `broker_sidecar._TickRelay` (та сама сесія що M1 fetch).
...
│  Процеси: broker_sidecar (tick relay V2), tick_preview_worker  │
...
| **Symbols** | `config.json → symbols` | 4 активних FXCM (XAU/USD, XAG/USD, NAS100, SPX500); Binance OFF з 05.09.2026, план хвиль — ADR-0054 §3.4 |
...
flowchart LR
    ...
    subgraph Tick["Tick pipeline"]
        TP[broker_sidecar<br/>_TickRelay V2<br/>BID mode]
...
```mermaid
flowchart TD
    A[app/main.py] -->|spawn| D[tick_preview 🟡 non_critical]
...
│   ├── main.py                    # supervisor (--mode all/m1_poller/broker_sidecar/tick_preview/binance_ingest_worker/binance_tick_publisher/ws_server/replay)
│   └── __init__.py
...
│   │   ├── broker/
│   │   │   ├── fxcm/
│   │   │   │   └── provider.py    # FxcmHistoryProvider (FXCM History API, PREVIOUS_CLOSE — ADR-0100)
│   │   │   └── binance/
...
│   │   ├── tick_publisher_fxcm.py # [DEPRECATED] FXCM tick publisher — тепер інтегровано в broker_sidecar._TickRelay (V2)
...
| **non_critical** | tick_preview, binance_tick_publisher | base=5s, max=120s | 10 | видаляється з пулу, інші працюють |
...
1. Supervisor запускає процеси (config-gated): broker_sidecar, m1_ingestion_worker, tick_preview_worker, binance_ingest_worker, binance_tick_publisher, ws_server.
...
### 4) Tick preview (broker_sidecar tick relay V2 + tick_preview_worker)

1. FXCM ForexConnect OFFERS table → broker_sidecar._TickRelay → Redis PubSub (`{ns}:price_tick`).
2. tick_preview_worker: schema guard → TickAggregator → UDS preview keyspace.
3. UI читає preview_curr для формуючого бару.

> **Важливо**: `tick_publisher_fxcm` зупинений назавжди (FXCM SDK dual-session conflict).
> Тіки FXCM йдуть через broker_sidecar (та сама сесія що M1 fetch).
