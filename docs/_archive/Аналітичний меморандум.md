...
> **ЗАСТАРІЛЕ**: Цей документ замінено на **[ADR-0003 Cold Start Hardening](ADR-0003%20Cold%20Start%20Hardening.md)**.
> Зберігається як історичний контекст. Актуальний план — в ADR-0003.
>
> **Що змінилось** (2026-02-19):
>
> - S2 (DeriveEngine warmup) — **DONE** (20260219-021)
> - Redis priming M1-H4 — **DONE** (20260219-027)
> - P-slices перераховані з урахуванням фактичного стану коду
> - Додано file:line evidence до кожної проблеми
...
3. **Derived Rebuild (M1 → HTF):**
    - Після завершення M1-завантаження викликається `tools/rebuild_from_m1.py` (канонічний rebuild).
    - Генерація M3, M5, M15, M30, H1, H4 виконується локально без запитів до брокера (Invariant I1).
4. **Gate & Priming:**
...
1. **I3 (Final > Preview):** Результати `rebuild_from_m1` завжди мають статус `complete=true` і затирають будь-які попередні preview-дані.
...
- [ ] **Pipeline:** Інтегрувати `rebuild_from_m1` як функціональний виклик, а не зовнішній скрипт.
