...
| `symbols` | str[] | `["XAU/USD", ...]` | **Список всіх активних символів** (4 шт: XAU/USD, XAG/USD, NAS100, SPX500). Визначає що збирається/показується |
...
| `m1_poller.session_open_rebuild.enabled` | bool | true | ADR-0096 слайс E: перша M1 після перерви перебудовується з тікової історії брокера (t1) до коміту. `false` = бар брокера як є (запечений open) |
...
| `m1_poller.session_open_rebuild.max_volume_deficit` | int | 5 | Скільки одиниць `v` брокер може мати понад кількість тіків t1 (запечений «тік» теж у `v`); більше — тіки неповні, бар іде з `open_provisional` |
| `m1_poller.session_open_rebuild.price_step_by_symbol` | {sym: float} | `{"XAU/USD": 0.01, "XAG/USD": 0.001, …}` | Крок котирування FXCM (10^-digits); допуск звірки close = ½ кроку. **Новий символ** без кроку → WARN `M1_SESSION_OPEN_REBUILD_NO_PRICE_STEP`, перші хвилини з `open_provisional` |
