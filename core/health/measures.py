...
def check_anchor_on_session_edge(
    ...
    *,
    ...
    Правильний якір: його перша хвилина торгується, а попередня — ні
    (``market_calendar`` тримає перерву напіввідкритою, тож 22:00 = перша торгова).
    Зсунутий якір ріже добу навпіл — саме так колись «поїхали» D1-свічки.
    """
    if tf_ms <= 0:
        return False
    return is_trading_fn(anchor_open_ms) and not is_trading_fn(anchor_open_ms - 60_000)
