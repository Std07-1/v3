...
SWING_KINDS = frozenset(
    {
        ...
        "bos_bear",
        "choch_bull",
        "choch_bear",  # Structure events (§4.2)
        "inducement_bull",
        "inducement_bear",  # Inducement (§4.7)
        "fractal_high",
        "fractal_low",  # Williams fractals (display-only)
        "displacement_bull",
        "displacement_bear",  # Displacement candles (momentum)
    }
)

# ADR-0035: Session High/Low level kinds
SESSION_LEVEL_KINDS = frozenset(
    {
        # Asia session
        "as_h",
        "as_l",  # Active Asia High/Low (running)
        "p_as_h",
        "p_as_l",  # Previous Asia High/Low (locked)
        # London session
        "lon_h",
        "lon_l",  # Active London High/Low
        "p_lon_h",
        "p_lon_l",  # Previous London High/Low
        # New York session
        "ny_h",
        "ny_l",  # Active New York High/Low
        "p_ny_h",
        "p_ny_l",  # Previous New York High/Low
    }
)
...
class SmcDelta:
    ...
    def has_changes(self) -> bool:
        return bool(
            self.new_zones
            or self.mitigated_zones
            or self.updated_zones
            or self.new_swings
            or self.new_levels
            or self.removed_levels
...
class NarrativeBlock:
    """Повний narrative для одного symbol+viewer_tf (ADR-0033 Rev 2, ADR-0035).
