...
| 0054 | Multi-Symbol Re-Activation Plan | Accepted (rev 3) | multi-symbol, regression net, isolation probe (рестарти+дублікати), Фаза 0 offline seed virgin FXCM, erratum ADR-0038, `has_range` HK/FX blocker, DST-freeze, per-symbol weekly rollout, supersedes ADR-0025 rollback |
...
> 0050 пропущено (TDA timing fix отримав номер 0051).
...
| X40 | **Platform reads the client, never the reverse**: platform-код (`runtime/`, `core/`, `ui_v4/`) що імпортує, читає або пише файли чи стан зовнішнього клієнта (`agent_bridge.data_dir`, directives, chat, owner-note) поза `runtime/agent_bridge/` (ADR-0090 §3.1). Платформа = торгова платформа; клієнт читає її через `/ws`, `/api/v3/*`, bridge — не навпаки |
