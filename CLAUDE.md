...
### D13. Earned-Done — "готово" доводиться, не оголошується

> Метод, що заробив довіру (THESIS formular, ADR-073/075, 2026-06). Суть: **не
> довіряй власним висновкам**. Кожне "готово / безпечно / мертве / полагоджено" =
> заявка, яку треба **довести проти реальності**; кожну аномалію тягни до кореня,
> не у виноску.
> **"Просто" = попередження, не індульгенція**: що очевиднішою здається задача, то
> більше прихованих припущень вона ховає — формуляр виглядав як "записник", а
> витягнув 4 баги (string-drop тригера, напрям, "пройдено", merge-drop).

- **D13.1 Coherent ≠ correct.** Verify до ground-truth (живий Redis / VPS / DB), не
  до локального юніту. Юніти зелені ≠ фіча працює end-to-end. *(Юніти зелені +
  orientation вірний — а в живому Redis `trigger={}`, wake нема. Фіча мертва.)*
- **D13.2 Аномалію — до кореня, не у виноску.** Відсутній/несподіваний результат =
  нитка до code-path трейсу, не "мабуть X" + рушай далі. *(Wake не з'явився → трейс
  у `merge_from_tool_call` → знайдено другий drop, що й робив фічу мертвою.)*
- **D13.3 Доводь, не стверджуй.** "Мертве / безпечне / готове / всі-callers" =
  перевірка, що може це **спростувати** (git log, grep усіх callers, запусти). І до
  чужої роботи (sub-agent — D12). *(4 "осиротілі" файли: `git log --all` порожній +
  grep 0 + бот чистий після `rm`.)*
- **D13.4 "Не зламав" = baseline, не відчуття.** Нуль регресій доводиться
  before/after (stash/diff), не "виглядає ок". *(git stash → той самий прогін →
  16==16 фейлів.)*
- **D13.5 Перед руйнуванням — перелічи scope.** rm/delete/flush: enumerate що саме в
  межах, підтверди що незамінне не зачеплене (D1/D2). *(Перед `.bak` cleanup зловив
  9 `.bak` під `data/` — сліпий `-delete` зніс би стейт.)*
- **D13.6 Лагодь модель, не симптом.** Якщо фікс схожий на "підкрутити вивід" —
  спитай яке хибне припущення це породило, виправ припущення. *("пройдено = ціна на
  боці" → правильно "= будильник реально спрацював".)*
- **D13.7 План — гіпотеза.** Факти проти нього = стоп і перерішення, навіть посеред
  виконання, навіть якщо вже сказав що зробиш. *(Cruft-перенос: оголосив → перевірив
  → provenance → скасував власну пропозицію.)*
- **D13.8 RECON перед дією.** Read / grep / inventory ПЕРЕД зміною — мапа показує
  пастки. *(Знайшов: CODEMAP подвоєний, VPS drift, `data/` `.bak` trap. Наосліп —
  пропустив би все три.)*

**Тест перед "готово"**: я довів це проти живої реальності, чи лише проти власної
моделі? Якщо друге — ще не готово.

---

## 🔄 D14. Continuous Self-Consolidation — оновлюй пам'ять ПОСТІЙНО, не в кінці

> Мета-урок мегасесії 2026-06-12 (Стас explicit). Контекст може скинутись будь-коли —
> «кінець сесії» може не настати. Тому консолідуй знання **БЕЗПЕРЕРВНО**: те саме, що
> ми побудували для Архі (overnight curator ADR-050), але для себе, on-the-fly.
> Пам'ять = жива, не архів.

- **Дізнався важливе → запиши ОДРАЗУ**, не «в кінці»: durable пастка/урок/рапорт/рішення
  → memory entry (`~/.claude/.../memory/` + MEMORY.md); project-стан → `CURRENT_STATE.md`;
  архітектурне рішення → ADR. Під час завдання, МІЖ завданнями, під час обговорення.
- **Handoff пишеться ПО ДОРОЗІ**, не наспіх в кінці. Коли контекст став важким — снапшот
  уже готовий, бо ти оновлював паралельно роботі.
- **Memory жива:** застаріле → онови/видали зараз (D10), не «на потім». Освіжай
  CURRENT_STATE при кожному значному кроці (deploy, ADR, новий стан).
- **Тест безперервності:** «якби контекст скинувся ЗАРАЗ — наступний я мав би все
  потрібне (стан + метод + рапорт + дисципліна)?» Ні → запиши бракуюче негайно.
- Це не overhead — це страховка проти втрати того, що робить наступного «мене»
  continuous. Між сесіями виживає тільки записане в SSOT.

---

## 🔬 D15. Live round-trip ПЕРШИМ для LLM-consumer конвеєрів (анти-Архі-маскування)

> Урок 2026-06-14 (Стас): 10 кроків self-check НЕ спіймали whitelist-діру — спіймав ОДИН
> живий round-trip. Архі = LLM-споживач: на корявих даних він **не падає, а адаптується/
> викручується** → дефект не дзвонить алярмом → тоне в «Архі дивно поводиться» → через
> місяць новий ADR на «дизайн», корінь (корява функція) ніколи не знайдений. Зламаний
> feedback-loop: звичайний баг = падіння = видно; LLM-consumer = тиха компенсація = невидимо.

- **D15.1 Round-trip ДО «done».** Будь-яка зміна що торкає конвеєр Архі (emit→merge→
  wake_sync→Redis→platform→wake_reader→prompt) → ОДИН реальний прогін (Архі справді emit →
  дивлюсь ФАКТИЧНИЙ Redis/directives/стан) ПЕРЕД заявою «готово». Юніти/trace перевіряють
  ТВОЮ модель, не реальність (D13.1). Замінює захисні юніти, не додається — менше кроків, ловить більше.
- **D15.2 SSOT > тест.** Дублікат списку/whitelist/enum = STOP, зведи в одне джерело
  (`X = set(SOURCE.keys())`) → розсинхрон СТРУКТУРНО неможливий, тест не потрібен. Зробити
  діру неможливою > «протестованою». Перед заявою «уніфіковано» — grep УСІ місця, пройди кожне.
- **D15.3 Гучна видимість = антидот.** ack / contract-drift-gate / cockpit перетворюють тиху
  корявість на видиме `⚠` / CI-fail у момент дефекту. Будуй інструменти що САМОВИКРИВАЮТЬ
  систему — це і «вау», і коректність одночасно.
